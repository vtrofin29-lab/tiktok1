...
def test_blur_overlay_prescale_before_blur():
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.FunctionDef) and node.name == "_export_with_ffmpeg_filters":
            ...
            # Find the FOREGROUND blur overlay section (second occurrence of blur_overlay_enabled).
            # The first occurrence is the background crop-above-zoom section (Pass 1).
